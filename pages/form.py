...
with st.form(key="form"):
    ...
    slider_input = st.slider("How much do you like the White Album?", key="white_album_rating")
    ...
    def validate_data():
        ...
        if len(name_input) == 0:
            name_warning = st.warning("Please input your name")
        elif slider_input == 0:
            slider_warning = st.warning("Please input a value greater than 0")
        else:
            st.session_state.submit_success = True

    submit_button = st.form_submit_button("Submit", on_click= validate_data)
